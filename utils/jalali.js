'use strict';
/* Jalali <-> Gregorian conversion — ported from the original local app so both
   share identical date behavior. */
...
function jalaliStrOf(jy, jm, jd) { return jy + '/' + pad2(jm) + '/' + pad2(jd); }
function todayJalali() {
  var now = new Date();
  return toJalaali(now.getFullYear(), now.getMonth() + 1, now.getDate());
}
function dateKey(jy, jm, jd) { return jy * 10000 + jm * 100 + jd; }
// week/month/year "bucket" keys for grouping in performance reports
function jalaliWeekKey(jy, jm, jd) {
  var jdn = j2d(jy, jm, jd);
  var weekNum = Math.floor(jdn / 7);
  return 'w' + weekNum;
}
function jalaliMonthKey(jy, jm) { return jy + '-' + pad2(jm); }
function jalaliYearKey(jy) { return '' + jy; }

module.exports = {
  toJalaali, toGregorian, jalaaliMonthLength, PERSIAN_MONTHS,
  jalaliStrOf, todayJalali, dateKey, pad2,
  jalaliWeekKey, jalaliMonthKey, jalaliYearKey, j2d, d2j
};
